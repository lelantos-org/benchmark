...
export const defaultProfile = (): NetworkProfile => NETWORK_PROFILES[1];
...
export function effectiveGamma(requested: number, noteCount: number): number {
    ...
    const byDecoys = budget < 2 ? GAMMA_MIN : Math.floor(Math.log2(budget));
    return Math.max(GAMMA_MIN, Math.min(requested, MAX_DETECTION_GAMMA, byDecoys));
}
...
export function ownIds(own: number, total: number): number[] {
    const ids: number[] = [];
    for (let k = 0; k < own; k++) ids.push(Math.max(1, Math.round(((k + 0.5) * total) / own)));
    return [...new Set(ids)];
}
