 ...
 */
function leafOf(P: Poseidon, J: Jubjub, note: Note): Field {
...
function buildWitness(P: Poseidon, J: Jubjub, circuit: Circuit): CircomTransactInput {
    ...
    const leafIndex = tree.insert(leafOf(P, J, spent));
    ...
    const inputs = [
        ...
        ...Array.from({ length: circuit.nIn - 1 }, (_, i) =>
            dummyInputAt(P, circuit.depth, BigInt(i + 1)),
        ),
    ];
